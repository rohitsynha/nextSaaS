...
export const appRouter = router({
    ...
    createStripeSession: privateProcedure.mutation(async ({ ctx }) => {
        ...
        if (subscriptionPlan.isSubscribed && dbUser.stripeCustomerId) {
            const stripeSession = await stripe.billingPortal.sessions.create({ customer: dbUser.stripeCustomerId, return_url: billingUrl })
        ...
        const stripeSession = await stripe.checkout.sessions.create({
            ...
            line_items: [{
                price: getPlanConfig('pro')?.price.priceIds.test,
                quantity: 1,
            }],
            metadata: {
                userId: userId,
