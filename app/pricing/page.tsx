...
import { getPlanConfig } from "@/config/stripe";
    ...
    const pricingItems = [
        {
          ...
          quota: getPlanConfig('free').quota,
          features: [
            {
              text: `${getPlanConfig('free').quota} pages per PDF`,
              footnote:
                'The maximum amount of pages per PDF-file.',
            },
            {
              text: `${getPlanConfig('free').maxSizeInMB}MB file size limit`,
              footnote:
        ...
        },
        {
          ...
          quota: getPlanConfig('pro').quota,
          features: [
            {
              text: `${getPlanConfig('pro').quota} pages per PDF`,
              footnote:
                'The maximum amount of pages per PDF-file.',
            },
            {
              text: `${getPlanConfig('pro').maxSizeInMB}MB file size limit`,
              footnote:
                'The maximum file size of a single PDF file.',
            },
            {
                ...
                <div className='pt-12 grid grid-cols-1 gap-10 lg:grid-cols-2'>
                    <TooltipProvider>
                        {pricingItems.map(({ plan, tagline, quota, features }) => {
                            const price = getPlanConfig(plan.toLowerCase())?.price.amount || 0

                            return (
                                <div key={plan} className={cn('relative rounded-2xl bg-white shadow-lg', {
                                    'border-2 border-rose-600 shadow-blue-200': plan === 'Pro', 'border border-gray-200': plan !== 'Pro'
                                })}>
                                    {plan === 'Pro' && (
                                        <div className='absolute -top-5 left-0 right-0 mx-auto w-32 rounded-full bg-gradient-to-r from-rose-600 to-fuchsia-600 px-3 py-2 text-sm font-medium text-white'>
                                            Upgrade now
                                        </div>
                                    )}
